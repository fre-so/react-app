...
- **Color variables**: Colors in `src/index.css` use `oklch()`; in JS/JSX (e.g., Recharts/inline styles) use `var(--chart-1)` directly—never wrap with `hsl()`/`rgb()`. Example: `fill="var(--chart-1)"` ✅, not `fill="hsl(var(--chart-1))"` ❌ (invalid, renders black).
